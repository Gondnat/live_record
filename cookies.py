...
def load_cookies(cookie_file, browser_specification):
    try:
        ...
        if browser_specification is not None:
            ...
            cookie_jars.append(
                extract_cookies_from_browser(browser_name, profile, logging, keyring=keyring, container=container))
...
def parse_safari_cookies(data, jar=None, logger=logging):
    """
    References:
...
'''
        ...
        with self.open(filename) as f:
            for line in f:
                try:
                    ...
                    logging.warning(f'WARNING: skipping cookie file entry due to {e}: {line!r}\n')
                    continue
        cf.seek(0)
    ...
    def get_cookies_dict_for_url(self, url):
        """Generate a dictionary of Cookie objects for a given url"""
        return {cookie.name: cookie.value for cookie in self.get_cookies_for_url(url)}
